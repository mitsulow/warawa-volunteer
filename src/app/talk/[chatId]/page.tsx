...
export default function TalkPage({
  ...
  if (!session.loading && !myId) {
    return (
      <main className="p-6 text-center">
        ...
        <Link href="/" className="text-[#c94d3a] font-bold underline">
          ← トップへ戻る
        </Link>
      </main>
    );
  ...
  return (
    <main className="flex flex-col h-dvh">
      <header className="flex items-center gap-3 px-4 py-3 bg-[#c94d3a] text-white sticky top-0">
            ...
            >
              <div
                className={`max-w-[75%] rounded-2xl px-3 py-2 text-sm whitespace-pre-wrap break-words ${
                  mine ? "bg-[#c94d3a] text-white" : "bg-white shadow-sm"
                }`}
              >
                {m.body}
                <span
      ...
      <div className="flex gap-2 p-3 bg-white border-t border-gray-200">
        ...
        />
        <button
          className="rounded-xl bg-[#c94d3a] px-4 text-white font-bold disabled:opacity-50"
          disabled={busy}
          onClick={send}
        >
          送信
