...
### 2026-08-16 の変更（新しい順）
- **オレンジ軍団15人確定(2026-09-01)**: 事務局選出メアド16件をユーザーから受領→auth.users照合で15人を body offer confirmed に（14人はopen→confirmed、やぎみほNo.3239はofferが無くトリガ停止で新規作成=自動コメントなし）。**それまで confirmed は0人だった**（8/25のメンバー決定時に確定処理がされていなかった）。1件 sasatac.2121@gmail.com は該当アカウント無し→別のGoogleアドレスでログインしている可能性・ユーザーに確認中。**リーダー=やぎみほさん(config.ts ORANGE_LEADER_ID)**: 現地入りメンバー欄で先頭+👑リーダー帯・現地報告投稿カードに「👑 オレンジ軍団リーダー」肩書き。照合スクリプトは個人メアドを含むため**repoに残さず削除済み**（公開repoのため）。**追加はDBで `offers.status` を open→confirmed に更新するだけ**（例: 2026-09-03 No.2335 ささき(ずんだ)＠宮城さんを追加→計17人）。**body offer が無い人は DO ブロックでトリガ disable→confirmed で insert→enable**（例: 2026-09-05 No.3424 セカイムラ熊本さん→計18人、同日 No.1 さとうみつろう本人も追加→計19人。管理者はRLS/canPost上もとから報告投稿可だが、現地入りメンバー欄に載せるため）
...
- **【重要】Management APIトークン失効(2026-09-01発覚)**: `~/.rakuichi-env` の SUPABASE_ACCESS_TOKEN が401。dbq.py は使えない。代替 `scripts/dbpg.py`(pg8000 で pooler aws-0-ap-northeast-1 セッションモード5432 直接続・パスは ~/.warawa-env の SUPABASE_DB_PASS)でDDL/DML可能。**毎晩バックアップは8/28〜8/31失敗していた（401）→ backup.py を dbpg 直接続に書き換えて復旧・9/1分取得済み**。**同日夜に新legacy token「warawa-claude」発行済み（Chrome連携で実施・90日期限=2026-11-30頃切れる→また401になったら再発行）**。dbq.py も復活。バックアップは今後も dbpg 直接続のまま（トークン切れの影響を受けないため）
...
- **ホームは開くたび必ず「現地報告」タブ(2026-09-05・ユーザー指示)**: 前回タブの復元は廃止。`?tab=` 指定(☰メニュー・Talk戻り)だけ尊重。localStorage `warawa-tab4` は☰のハイライト用に書くだけで読まない
