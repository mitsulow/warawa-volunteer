...
- **友達申請(2026-08-16夜)**: `friendships`(requester/addressee/status pending|accepted・`scripts/friendships.sql`)。1対1TalKの新規作成(chats insert)は `can_talk(a,b)`=事務局ボット or 管理者が絡む or 友達承認済み のみ。マイページのボタンが状態で変わる(友達申請をする→申請中・取り消す／承認する・断る→TalKで連絡を取る)。申請/承認は🔔通知(kind friend_request/friend_accept・トリガ notify_friendship)。既存チャットは影響なし
